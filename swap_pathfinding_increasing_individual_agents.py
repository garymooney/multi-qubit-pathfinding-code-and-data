...
def generate_linear_program_gurobi_multi(
        time_expanded_graph, 
        start_names=["source"], 
        end_names=["target"], 
        time_limit = 1000., 
        allow_out_flow_less_than_in = False, 
        trim_impossible_variables = False,
        vtype = GRB.BINARY):
    """
    Parameters
    ----------
    vtype, optional
        The type for the variables (GRB.BINARY or GRB.CONTINUOUS), by default GRB.BINARY
    """
    ...
    if trim_impossible_variables == True:
        ...
        for x in variables:
            if convert_var_to_possible_var[x] != None:
                flows.append(lp_problem.addVar(vtype=vtype, name=str(convert_var_to_possible_var[x])))
                #flows.append(LpVariable(str(convert_var_to_possible_var[x]), 0, 1, LpBinary))
    else:
        flows = [lp_problem.addVar(vtype=vtype, name=str(x)) for x in variables]
...
def multiple_group_commodity(
        initial_graph, 
        source_locations_list, 
        target_locations_list, 
        weights=None, 
        time_limit=1000., 
        output_solution_graph = True, 
        use_single_group_first = False, 
        allow_out_flow_less_than_in = False, 
        trim_impossible_variables = False, 
        problem_name = None, 
        use_gurobi = False, 
        approximate_min_time_depth = False):
...
def main():
    ...
    pos = None
    devices = {"grid8x8": 64, "ibmq_melbourne": 15, "ibmq_paris": 27, "ibmq_poughkeepsie": 20, "ibmq_rochester": 53, "rigetti_acorn": 20}

    for device_name, device_qubit_count in devices.items():
        if device_name == "grid8x8":
            initial_graph = relabel.convert_node_labels_to_integers(nx.grid_2d_graph(8, 8))
            pos = None
        if device_name == "ibmq_melbourne":
            initial_graph, pos = create_ibmq_melbourne_hardware_graph()
        if device_name == "ibmq_paris":
            initial_graph, pos = create_ibmq_paris_hardware_graph()
        if device_name == "ibmq_poughkeepsie":
            initial_graph, pos = create_ibmq_poughkeepsie_hardware_graph()
        if device_name == "ibmq_rochester":
            initial_graph, pos = create_ibmq_rochester_hardware_graph()
        if device_name == "rigetti_acorn":
            initial_graph, pos = create_rigetti_acorn_hardware_graph()

        print("")
        print("device: " + str(device_name))
        print("qubit_count: " + str(device_qubit_count))
        ...
        #plt.show(block=False)
        plt.savefig(f"initial_graph-{device_name}.png", format="png")
        plt.savefig(f"initial_graph-{device_name}.svg", format="svg")
        plt.clf()

        results_directory = f"results-{device_name}-increasing-error_stds"
        
        max_agent_count = device_qubit_count
        #max_split_count = 24
        ...
        for number_of_agents in range(1, max_agent_count+1):
            ...
            for i in range(number_of_iterations):
                print("")
                print("agent_count:", str(number_of_agents))
